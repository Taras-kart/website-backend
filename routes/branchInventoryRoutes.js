...
router.post('/:branchId/import', requireBranchAuth, upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    for (const raw of rows) {
      ...
      if (!ProductName || !BrandName || !SIZE || !COLOUR) { err++; continue }
      ...
      try {
        ...
        await client.query(
          `INSERT INTO import_rows (import_job_id, raw_row_json, status_enum, error_msg)
           VALUES ($1,$2,'ERROR',$3)`,
          [job.id, raw, String(e.message || 'error').slice(0, 500)]
        )
      }
    }
    ...
    await client.query(
      `UPDATE import_jobs
       ...
       WHERE id = $5`,
      [status, total, ok, err, job.id]
    )

    res.status(201).json({ ...job, status_enum: status, rows_total: total, rows_success: ok, rows_error: err })
  } catch {
    res.status(500).json({ message: 'Server error' })
  } finally {
    client.release()
