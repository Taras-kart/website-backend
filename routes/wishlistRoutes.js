  ...
  try {
    ...
    const sqlNoBranch = `
      ...
      SELECT
        ...
        pv.gender,
        pv.mrp AS original_price_b2c,
        COALESCE(NULLIF(pv.sale_price,0), pv.mrp) AS final_price_b2c,
        pv.mrp AS original_price_b2b,
        COALESCE(NULLIF(pv.cost_price,0), COALESCE(NULLIF(pv.sale_price,0), pv.mrp)) AS final_price_b2b,
        COALESCE(
          NULLIF(pv.v_image,''),
          NULLIF(pv.pi_image,''),
          CASE
    ...
    try {
      const { rows } = await pool.query(sqlWithBranch, [cloud, branchId, userId]);
      ...
      try {
        const { rows } = await pool.query(sqlNoBranch, [cloud, userId]);
        return res.json(rows);
      } catch (err2) {
        return res.status(500).json({ message: 'Error fetching wishlist', error: err2.message });
      }
    }
  } catch (err) {
